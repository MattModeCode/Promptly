...
private final class DetailViewController: NSViewController {
    ...
    var isDirty: Bool {
        titleField.stringValue != baselineTitle ||
            ...
            pinButton.isOn != baselinePinned ||
            hotkeyField.stringValue != baselineHotkey ||
            currentFolder != baselineFolder
    }
    ...
    /// Just remembers the target folder — saving is explicit now, so the actual
    /// `PromptStore.move(_:toFolder:)` (which migrates the usage key so frecency survives the
    /// reorganize, DESIGN §7.1) happens in `persist()` along with everything else on Save.
    ...
    /// Writes title/description/body/pinned/hotkey through `PromptStore.save(...)`, called only
    /// from the explicit Save button — there is no auto-save on field blur. Also applies any
    /// pending folder change (deferred by `applyFolderChange`) via `PromptStore.move(_:toFolder:)`
    /// first, so the usage key migrates before the rest of the fields are written. Hotkey
    /// conflicts resolve by a user-initiated steal — distinct from Stage 8's silent, read-only
    /// load-time handling: here the user explicitly claimed the slot, so the prior holder's file
    /// IS rewritten, and the inline warning says so.
    private func persist() {
        ...
        if !currentFilename.isEmpty,
           let existing = promptStore.prompts.first(where: { $0.filename == currentFilename }),
           existing.folder != currentFolder {
        ...
        promptStore.save(name: name, keywords: currentKeywords, body: body, folder: currentFolder,
                         pinned: pinned, hotkey: hotkey, description: description.isEmpty ? nil : description,
...
final class LibraryWindowController: NSWindowController {
    ...
    private func wire() {
        ...
        listVC.onSelectPrompt = { [weak self] prompt in
            guard let self else { return }
            guard self.confirmDiscardIfNeeded() else {
    ...
    /// Saving is explicit now (no auto-save on blur) — without this guard, clicking another
    /// prompt or starting a new one would silently discard in-progress edits. Returns true if
    /// it's safe to proceed (nothing unsaved, or the user chose to discard).
    private func confirmDiscardIfNeeded() -> Bool {
        guard detailVC.isDirty else { return true }
        let alert = NSAlert()
        alert.messageText = "Discard unsaved changes?"
        alert.informativeText = "Your edits to this prompt have not been saved."
