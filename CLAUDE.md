...
- **Typecheck gate:** `arch -x86_64 swiftc -typecheck PasteProbe.swift` — must exit 0. *(Runs today.)*
...
each roadmap stage has a self-contained execution file in **`docs/stages/`**; how to build a stage
with subagents — gate-aware and bound to the Tier A/B + honesty rules above — is in
